...
RunWait, cmd.exe /c start /b %A_ScriptDir%\deeplx.exe /s, , Hide
...
^1::
  ...
  {
    TrimLine := Trim(A_LoopField)
    If (TrimLine != "")
      selectedTexts.push(TrimLine)
  }
  
  encodedTexts := ""
  
  Loop % selectedTexts.Length()
  {
    encodedLine := StrReplace(selectedTexts[A_Index], "`n", "\n")
    encodedLine := StrReplace(encodedLine, """", "\""")
    encodedLine := StrReplace(encodedLine, "`r", "\n")
    encodedTexts := encodedTexts ? encodedTexts . "" . encodedLine : encodedLine
  }

  http.Open("POST", "http://127.0.0.1:1188/translate", false)
  http.SetRequestHeader("Content-Type", "application/json")
  body = {"text": "%encodedTexts%", "target_lang": "ZH"}
  ;MsgBox, % body
  http.Send(body)
  response := http.ResponseText
  jsonData := JSON.parse(response)
  data := jsonData["data"]
  Gui, Add, GroupBox, x10 y10 w280 h360, 翻译结果
  Gui, Font, s10
  Gui, Add, Edit, x20 y35 w260 h320 vOutputText ReadOnly cBlack BackgroundTrans +VScroll, %data%
  GuiControl, +0x200000, OutputText
  Gui, Add, Button, x20 y360 w260 gCopyButton, 复制并关闭
  Gui, Show, w300 h400, DeepLx
return

GuiSize:
    GuiControl, Focus, TextCtrl
    SendMessage, 0xB1, 0, 0, Edit1, ahk_class AutoHotkeyGUI
return

CopyButton:
    GuiControlGet, text, , OutputText
    Clipboard := text
    Gui, Destroy
return
